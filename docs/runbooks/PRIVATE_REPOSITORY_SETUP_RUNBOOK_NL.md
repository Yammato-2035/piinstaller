> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/runbooks/PRIVATE_REPOSITORY_SETUP_RUNBOOK_EN.md`). Bitte bei Release manuell gegenlesen.

# Runbook — Private Repository Setup (EN)

**As of:** 2026-06-16  
**Audience:** Maintainers with access to public and private repositories  
**Status:** Process Documentatie — Neet an automated Deploy

---

## 1. Purpose

Step-by-step guide to create and maintain a **private Setuphelfer repository** for Cloudserver Edition, telemetry server, diagNeestics server, operator dashboard, and commercial modules — separate from the **public** `piinstaller` repository.
...
| 3 | Secret manager or CI vault (Nee secrets in git) |
...
2. Protect default branch `main` (PR requirood, status checks).
3. Do **Neet** mirror the entire public tree — selective integration only.
...
Expect Nee exit 10–16 (geblokkeerd). Exit 20 = manual review.
...
Documentatie placeholder domains: `*.setuphelfer.example`
...
| Telemetry server | `TELEMETRY_Intern_SERVER_HANDOFF.md` | [ ] |
| DiagNeestics server | `DIAGNeeSTICS_Intern_SERVER_HANDOFF.md` | [ ] |
...
5. Nee auto-Deploy to production without runbook approval
...
## 10. Incident and rollTerug

| Scenario | Action |
|----------|--------|
| Secret leaked in public repo | Rotate keys, scrub history (owner), extend gate |
| Private code in public PR | Sluiten PR, review boundary gate |
...
- German NDA/beta items: `NDA_REQUIrood_ITEMS_DE.md`, `BETA_AGREEMENT_REQUIrood_ITEMS_DE.md`
...
- [ ] Nee production testing against `/opt` without Phase 0 runtime gate
