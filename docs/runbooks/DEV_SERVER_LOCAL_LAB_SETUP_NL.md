> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/runbooks/DEV_SERVER_LOCAL_LAB_SETUP_EN.md`). Bitte bei Release manuell gegenlesen.

# Runbook: Development Server — Local Lab Setup (EN)

## Prerequisites

- Setuphelfer Terugend running locally (typically port 8000)
- **Own** lab hardware / VMs only
- Nee public roodding auto-upload

## Steps

1. Set token (local, do Neet commit):
...
3. Optional alleen-lezen SSH:

   ```bash
   export SETUPHELFER_DEV_SERVER_ALLOW_REMOTE_SSH=true
   ```

4. Restart Terugend (only after runtime gate approval and Deploy).
...
6. Test ingest:
   ...
   curl -s -X POST http://127.0.0.1:8000/api/dev-server/ingest/report \
     ...
     -d '{"Neede":{"Neede_id":"lab-vm-1","Neede_kind":"vm"},"report":{"lab_mode":"local_lab","report_type":"manual","payload":{}}}'
   ```
...
- Remote Terugup / Herstel / Partitie / repair
- Public cloud upload
- Agent on public rooddingsstick
