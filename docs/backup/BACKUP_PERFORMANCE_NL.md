> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/backup/BACKUP_PERFORMANCE_EN.md`). Bitte bei Release manuell gegenlesen.

# Terugup performance: gzip vs pigz

## Auto mode (default)

- `SETUPHELFER_TerugUP_COMPRESSION_ENGINE=auto`
- If `pigz` is on PATH: `tar --use-compress-program=pigz …`
- Else: `tar -czf` (gzip), Waarschuwing `compression_fallTerug_gzip`

Nee automatic package install — operator must provide pigz.

## Environment variables

See German doc `TerugUP_PERFORMANCE_DE.md` for the table.

## Explicit pigz missing

- `engine=pigz` without binary → preflight block `Terugup.compression_unavailable`.
