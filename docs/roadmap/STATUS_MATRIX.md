...
| Development Cockpit **DEV-001** | Gelb | `GET /api/dev-dashboard/*` read-only; Rescue-ISO-Executor mit kontrollierten POST-Steps unter `/api/dev-dashboard/rescue-iso/*`; neuer kontrollierter Deploy-Helper (`/api/dev-dashboard/deploy/*`) mit `systemd`-Oneshot-Architektur, persistenter Deploy-Evidence und lokaler Update-Karte vorbereitet; UI nur Entwickler; **Runtime vs. Workspace** + **`deploy_drift`** (Whitelist + optionales Deployment-Manifest); produktive Helper-Installation/Runtime-Smokes noch offen | `docs/testing/DEVELOPMENT_COCKPIT_MATRIX.md`, `docs/dev-dashboard/README.md`, `docs/dev-dashboard/DEV_CLIENT_DE.md`, `docs/evidence/dev-dashboard/RESCUE_ISO_EXECUTOR_DASHBOARD_INTEGRATION_RESULT.md`, `docs/evidence/dev-dashboard/DEPLOY_HELPER_INTEGRATION_RESULT.md`, `backend/core/dev_dashboard.py`, `backend/core/deploy_manifest.py` |
...
| **Controlled ISO build execution** | Gelb | Clean-State + noauto fix; **tar/adduser** Ursache dokumentiert; Dashboard-Executor erkennt stale root-owned State, Operator-Clean und Build-Kommandos; `auto/clean`-Rekursion behoben; Runtime-Gate derzeit nicht grün, ISO weiter ausstehend | `RESCUE_CONTROLLED_ISO_BUILD_RESULT.md`, `RESCUE_ISO_EXECUTOR_DASHBOARD_INTEGRATION_RESULT.md` |
