...
| **DCC unter local_lab live** | Gelb / partial_green | Deploy `199d3c6`; API Dev-Routen HTTP 200; Operator: DCC + Boot-Diagnose sichtbar; **release restore ausstehend** für voll grün | `DCC_LIVE_ACCEPTANCE_AFTER_FALLBACK_OPERATOR_OBSERVATION.md`, `dcc_live_acceptance_after_fallback_latest.json` |
| **DCC Frontend Profile Desync (Gating)** | Gelb / partial_green | Gating-Fix deployed; local_lab Status 200 + Operator-DCC sichtbar; release restore für grün | `DCC_FRONTEND_PROFILE_DESYNC_RESULT.md`, `DCC_LIVE_ACCEPTANCE_AFTER_FALLBACK_OPERATOR_OBSERVATION.md` |
| **DCC Blank Screen Fail-Safe** | Gelb / partial_green | `blank_dcc_screen` resolved — Marker in `/opt` + served JS; Operator-Fallback sichtbar; kein leerer Browser mehr | `DCC_BLANK_SCREEN_TRIAGE_RESULT.md`, `DCC_LIVE_ACCEPTANCE_AFTER_FALLBACK_OPERATOR_OBSERVATION.md` |
