...
| **DCC Frontend Profile Desync (Gating)** | Gelb / review_required | Fix committed (`4fb72ee`); Deploy + local_lab Browser-Smoke ausstehend (sudo); `/opt` Frontend-Dist noch alt | `DCC_FRONTEND_PROFILE_DESYNC_RESULT.md`, `DCC_FRONTEND_PROFILE_DESYNC_LIVE_ACCEPTANCE_RESULT.md`, `dcc_frontend_profile_desync_live_acceptance_latest.json` |
