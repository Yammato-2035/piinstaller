...
| Development Cockpit **DEV-001** | Grün | `GET /api/dev-dashboard/*` read-only/observability grün; Rescue-ISO-Executor unter `/api/dev-dashboard/rescue-iso/*` produktiv verifiziert; kontrollierter Deploy-Helper (`/api/dev-dashboard/deploy/*`) via `systemd`-Oneshot produktiv installiert; Update-Status `ok`; neue read-only Endpunkte `packaging/readiness` und `project-overview` antworten produktiv; USB-Write und andere verbotene Aktionen bleiben blockiert | `docs/testing/DEVELOPMENT_COCKPIT_MATRIX.md`, `docs/dev-dashboard/README.md`, `docs/dev-dashboard/DEV_CLIENT_DE.md`, `docs/evidence/dev-dashboard/RESCUE_ISO_EXECUTOR_DASHBOARD_INTEGRATION_RESULT.md`, `docs/evidence/dev-dashboard/DEPLOY_HELPER_INTEGRATION_RESULT.md`, `docs/evidence/dev-dashboard/PROJECT_OVERVIEW_DASHBOARD_INTEGRATION_RESULT.md`, `backend/core/dev_dashboard.py`, `backend/core/deploy_manifest.py` |
| **Notification Dashboard** | Grün | Produktive Runtime verifiziert: Gate Exit `0`, neue `/api/dev-dashboard/notifications/*`-Endpunkte antworten unter `/opt/setuphelfer`, Rescue-Failure-Event und Dashboard-Testevent persistieren unter `/var/lib/setuphelfer/notifications` | `NOTIFICATION_MODULE_IST_ANALYSIS.md`, `NOTIFICATION_MODULE_INTEGRATION_RESULT.md`, `NOTIFICATION_EVENT_CONTRACT.md`, `/var/lib/setuphelfer/notifications/notification_latest_summary.json` |
| **Notification Email** | Gelb | SMTP ist produktiv konfiguriert und der echte Versandpfad wird ausgefuehrt, aber aktuelle Zustellung ist wegen Provider-Limit blockiert (`554 5.7.0 ... limit on the number of allowed outgoing messages was exceeded`) | `NOTIFICATION_MODULE_INTEGRATION_RESULT.md`, `NOTIFICATION_EMAIL_SETUP_RUNBOOK.md`, `/var/lib/setuphelfer/notifications/notification_events.jsonl`, `/var/lib/setuphelfer/notifications/notification_latest_summary.json` |
...
| **Controlled ISO build execution** | Rot | Prebuild/Executor weiter gruen, aber neuer echter `sudo lb build noauto` auf `fe36af0` scheitert mit `LB_EXIT=127` (`/usr/bin/env: 'rsvg': No such file or directory`); danach `sudo_clean_required`, USB bleibt blockiert | `RESCUE_CONTROLLED_ISO_BUILD_RESULT.md`, `RESCUE_ISO_EXECUTOR_DASHBOARD_INTEGRATION_RESULT.md`, `controlled_iso_build_latest_summary.json` |
...
| **Rescue USB write gate** | Rot | **blocked** — kein ISO, keine Zielgeraet-Auswahl, keine Doppelbestaetigung, kein `dd` | `RESCUE_USB_WRITE_GATE_RUNBOOK.md`, `RESCUE_USB_WRITE_RESULT.md`, `controlled_usb_write_latest_summary.json` |
