...
| **DCC Frontend Profile Desync (Gating)** | Gelb | Disabled-Page wird jetzt nur bei Block via `/api/dev-dashboard/status` angezeigt (no-store + `?t=`); unit-tests OK; live local_lab Check offen | `DCC_FRONTEND_PROFILE_DESYNC_RESULT.md`, `dcc_frontend_profile_desync_latest.json` |
