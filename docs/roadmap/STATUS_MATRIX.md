...
| **DCC Frontend Profile Desync (Gating)** | Gelb / review_required | Fix committed (`4fb72ee`); Deploy + local_lab Browser-Smoke ausstehend (sudo) | `DCC_FRONTEND_PROFILE_DESYNC_RESULT.md`, `DCC_FRONTEND_PROFILE_DESYNC_LIVE_ACCEPTANCE_RESULT.md` |
| **DCC Blank Screen Fail-Safe** | Gelb | `DccBootDiagnosticsPanel` + ErrorBoundary; Bundle-Marker `DCC_BOOT_DIAGNOSTICS_V1`; nie leere Seite — Deploy/local_lab ausstehend | `DCC_BLANK_SCREEN_TRIAGE_RESULT.md`, `dcc_blank_screen_triage_latest.json` |
