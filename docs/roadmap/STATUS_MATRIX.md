...
| **Controlled ISO build execution** | Rot | Letzter echter Build auf `fe36af0` scheiterte mit `LB_EXIT=127` (`/usr/bin/env: 'rsvg': No such file or directory`); neuer 1.7.2-Preflight blockiert den Build jetzt frueh als `blocked_build_tools_missing` und nennt `librsvg2-bin`; produktiver Runtime-Smoke zeigt weiter `status=red`, `next_operator_action=build_dependency_required`, `usb_write.allowed=false` | `RESCUE_CONTROLLED_ISO_BUILD_RESULT.md`, `RESCUE_ISO_EXECUTOR_DASHBOARD_INTEGRATION_RESULT.md`, `RESCUE_ISO_RSVG_FAILURE_ANALYSIS.md`, `controlled_iso_build_latest_summary.json` |
