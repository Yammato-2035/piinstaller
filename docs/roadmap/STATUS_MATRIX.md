...
| Development Cockpit **DEV-001** | Gelb | Deploy-Drift-False-Positive fuer `setuphelfer-backup-starter.py` per `2c7e9ee` behoben; **ein** Helper-Deploy nach `/opt` ausstehend, danach internes Runtime-Gate wieder grün erwartet; Rescue-ISO/USB-Write unveraendert blockiert bis echter Build | `docs/evidence/dev-dashboard/DEPLOY_DRIFT_SINGLE_FILE_FIX.md`, `docs/testing/DEVELOPMENT_COCKPIT_MATRIX.md`, `docs/dev-dashboard/README.md`, `docs/evidence/dev-dashboard/DEPLOY_HELPER_INTEGRATION_RESULT.md`, `docs/evidence/dev-dashboard/RESCUE_ISO_EXECUTOR_DASHBOARD_INTEGRATION_RESULT.md` |
