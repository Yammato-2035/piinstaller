...
| **Controlled ISO build precheck (no build)** | Gelb | Cleanup ok; Validate Exit 0 nach `dangerous_path_override`-Fix; **ready_for_controlled_iso_build_operator_run** — kein neues ISO/Boot in diesem Lauf | `controlled_iso_build_precheck_latest.json`, `RESCUE_ISO_VALIDATE_DANGEROUS_PATH_FIX_RESULT.md` |
