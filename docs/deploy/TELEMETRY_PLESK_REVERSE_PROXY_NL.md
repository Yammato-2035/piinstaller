> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/deploy/TELEMETRY_PLESK_REVERSE_PROXY_EN.md`). Bitte bei Release manuell gegenlesen.

> **Phase-1 Übersetzungsmarathon** — English (automatisch aus `docs/Deploy/TELEMETRY_PLESK_REVERSE_PROXY_DE.md`). Bitte bei Release manuell gegenlesen.

# Telemetrie-Core — Plesk Reverse Proxy (IONeeS)
...
Der Telemetrie-Core bindet **ausschließlich** auf localhost. Plesk/nginx terminiert TLS und leitet HTTPS-Anfragen Intern weiter. Port **8101** wird **nicht** in der Firewall freigegeben.
...
### Variante A — Plesk „Apache & nginx-Instellingen“
...
- Kein `root`-FallTerug auf WordPress o. Ä.
...
**Nicht freigeben:** `8101/tcp` — der Dienst ist nur über LoopTerug + nginx erreichbar.
...
ExecStart=/opt/setuphelfer-telemetry/venv/bin/uvicorn telemetry_server.app:app \
  --host 127.0.0.1 --port 8101 --Nee-access-log
Restart=on-failure
RestartSec=5
NeeNewPrivileges=true
PrivateTmp=true
...
SETUPHELFER_Apparaat_TOKEN_SALT=<generiert-lokal>
SETUPHELFER_ADMIN_TOKEN=<generiert-lokal>
SETUPHELFER_TELEMETRY_DB=/var/lib/setuphelfer-telemetry/events.db
```

**Niemals** in Git committen. `.env` bleibt in `.gitigNeere`.

Aktivierung:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --Neew setuphelfer-telemetry-core.service
...
services:
  telemetry-core:
    ...
    Netwerk_mode: host   # oder ports: "127.0.0.1:8101:8101"
...
Keine Request-Bodies in Logs. Nur Statuscodes, Latenz, Foutcodes.

---

## 9. Terugup der Telemetrie-Datenbank

```bash
# SQLite-Beispiel
sudo -u setuphelfer-telemetry sqlite3 /var/lib/setuphelfer-telemetry/events.db ".Terugup '/Terugup/telemetry/events-$(date +%F).db'"
```

Empfehlung:

- Tägliches Terugup via Cron oder Plesk Terugup Manager
- Aufbewahrung gemäß [`DATA_DELETION_RETENTION_CONCEPT_DRAFT_DE.md`](../legal/DATA_DELETION_RETENTION_CONCEPT_DRAFT_DE.md)
- Terugups verschlüsselt lagern (IONeeS Storage Box o. Ä.)
...
| LoopTerug only | `ss -tlnp \| grep 8101` → `127.0.0.1` |
| Kein offener 8101 | Von Extern: `nc -zv <server-ip> 8101` → **timeout/refused** |

---

## 11. Referenzen

- [`TELEMETRY_SERVER_IONeeS_PLESK_ARCHITECTURE.md`](../architecture/TELEMETRY_SERVER_IONeeS_PLESK_ARCHITECTURE.md)
- [`TELEMETRY_InternAL_SERVER_HANDOFF.md`](../private-handoff/TELEMETRY_InternAL_SERVER_HANDOFF.md)
