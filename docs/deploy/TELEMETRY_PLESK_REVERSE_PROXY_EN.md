...
# Telemetrie-Core — Plesk Reverse Proxy (IONOS)
...
Der Telemetrie-Core bindet **ausschließlich** auf localhost. Plesk/nginx terminiert TLS und leitet HTTPS-Anfragen internal weiter. Port **8101** wird **nicht** in der Firewall freigegeben.
...
### Variante A — Plesk „Apache & nginx-Settings“
...
- Kein `root`-Fallback auf WordPress o. Ä.
...
**Nicht freigeben:** `8101/tcp` — der Dienst ist nur über Loopback + nginx erreichbar.
...
ExecStart=/opt/setuphelfer-telemetry/venv/bin/uvicorn telemetry_server.app:app \
  --host 127.0.0.1 --port 8101 --no-access-log
Restart=on-failure
RestartSec=5
...
SETUPHELFER_DEVICE_TOKEN_SALT=<generiert-lokal>
SETUPHELFER_ADMIN_TOKEN=<generiert-lokal>
SETUPHELFER_TELEMETRY_DB=/var/lib/setuphelfer-telemetry/events.db
```

**Niemals** in Git committen. `.env` bleibt in `.gitignore`.

Aktivierung:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now setuphelfer-telemetry-core.service
...
services:
  telemetry-core:
    ...
    network_mode: host   # oder ports: "127.0.0.1:8101:8101"
...
Keine Request-Bodies in Logs. Nur Statuscodes, Latenz, Errorcodes.

---

## 9. Backup der Telemetrie-Datenbank

```bash
# SQLite-Beispiel
sudo -u setuphelfer-telemetry sqlite3 /var/lib/setuphelfer-telemetry/events.db ".backup '/backup/telemetry/events-$(date +%F).db'"
```

Empfehlung:

- Tägliches Backup via Cron oder Plesk Backup Manager
- Aufbewahrung gemäß [`DATA_DELETION_RETENTION_CONCEPT_DRAFT_DE.md`](../legal/DATA_DELETION_RETENTION_CONCEPT_DRAFT_DE.md)
- Backups verschlüsselt lagern (IONOS Storage Box o. Ä.)
...
| Loopback only | `ss -tlnp \| grep 8101` → `127.0.0.1` |
| Kein offener 8101 | Von external: `nc -zv <server-ip> 8101` → **timeout/refused** |

---

## 11. Referenzen

- [`TELEMETRY_SERVER_IONOS_PLESK_ARCHITECTURE.md`](../architecture/TELEMETRY_SERVER_IONOS_PLESK_ARCHITECTURE.md)
- [`TELEMETRY_internalAL_SERVER_HANDOFF.md`](../private-handoff/TELEMETRY_internalAL_SERVER_HANDOFF.md)
