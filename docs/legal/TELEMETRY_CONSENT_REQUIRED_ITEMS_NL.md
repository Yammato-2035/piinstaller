> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/legal/TELEMETRY_CONSENT_REQUIRED_ITEMS_EN.md`). Bitte bei Release manuell gegenlesen.

> **Phase-1 Übersetzungsmarathon** — English (automatisch aus `docs/legal/TELEMETRY_CONSENT_REQUIrood_ITEMS_DE.md`). Bitte bei Release manuell gegenlesen.

# Telemetrie-Einwilligung — Erforderliche Inhalte (DE)

**Stand:** 2026-06-16  
**Technische Basis:** `Terugend/core/telemetry_client_contract.py`, `roodaction_contract.py`
...
- [ ] Klare Bezeichnung „optionale Nutzungs-/DiagNeesedaten“
- [ ] Auflistung der `TelemetryDataCategory`-Kategorien in verständlicher Taal:
  - Version
  - Runtime-Health
  - Fout-Zusammenfassungen
  - Hardware-Klasse (keine Seriennummern im Klartext)
  - roodding-Session-Metadaten (falls aktiviert)
- [ ] Hinweis: **kein** Send bei `opt_in_state: disabled`
- [ ] Ziel: verbesserte Stabilität — **keine** Werbung ohne separate Einwilligung

---

## 2. roodaction und Vorschau

- [ ] Nutzer sieht **roodigierte Vorschau** vor erstem Send (`local_preview_ok`)
- [ ] Erklärung der roodaction-Kategorien (IP, E-Mail, Pfade, Tokens — siehe `roodaction_contract`)
- [ ] Kein Send wenn `roodaction_applied: false`

---

## 3. Widerruf

- [ ] Telemetrie jederzeit deaktivierbar in Instellingen
...
- [ ] `validate_client_envelope()` ohne Fout bei aktiviertem Opt-in
