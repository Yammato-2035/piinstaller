...
Nutzen Sie die **Raw-GitHub-URL** des Installer-Skripts (ersetzen Sie `OWNER` durch den Repository-Besitzer):

```bash
curl -sSL https://raw.githubusercontent.com/OWNER/PI-Installer/main/scripts/create_installer.sh | bash
```
...
1. **Option A – Redirect:** Die Domain (oder einen Pfad wie `https://get.pi-installer.io/install`) per HTTP-Redirect (302) auf die Raw-GitHub-URL zeigen lassen:
   ```
   https://raw.githubusercontent.com/OWNER/PI-Installer/main/scripts/create_installer.sh
   ```
...
```bash
PI_INSTALLER_REPO=https://github.com/OWNER/PI-Installer.git curl -sSL https://get.pi-installer.io | bash
```
