> **Phase-1 Übersetzungsmarathon** — Deutsch (automatisch aus `docs/architecture/CORE_FACADE_RULES_EN.md`). Bitte bei Release manuell gegenlesen.

# Core Fassade Regeln (Phase A.1 — Freeze)

**Stand:** ACTIVE — caller migration A.2–A.4 (safety) complete; boundary still warn-only globally.
...
| Rettung FAT32/ESP (`rescue_fat32_esp_*`) | Hardware write path with evidence | Dedicated rescue exception |
