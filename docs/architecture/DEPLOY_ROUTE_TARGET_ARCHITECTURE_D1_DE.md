> **Phase-1 Übersetzungsmarathon** — Deutsch (automatisch aus `docs/architecture/DEPLOY_ROUTE_TARGET_ARCHITECTURE_D1_EN.md`). Bitte bei Release manuell gegenlesen.

# Deploy Route Ziel Architektur (Phase D.1)

**Stand:** planning document — **no files created**, no refactoring in D.1.
...
| Ziel file | Purpose | ~routes | ~lines | Dependencies | Risks |
|-------------|---------|---------|--------|--------------|-------|
...
Already implemented via `runner_api_facade` (C.3/C.4). Extraktion is pure `include_router` with no behavior change.
