> **Phase-1 Übersetzungsmarathon** — Deutsch (automatisch aus `docs/architecture/DEPLOY_ROUTE_EXTRACTION_SEQUENCE_D7_PLUS_EN.md`). Bitte bei Release manuell gegenlesen.

# Deploy Route Extraktion Sequence D.7+
...
| **D.11** | `routes_runtime.py` | HIGH | read-only status/plan | `/execute`, write | unit | DE/EN | **done** — 8 routes |
...
1. Fassade-first before physical move
2. One sub-router per phase (max 6–10 routes)
3. Boundary guard stays green/review_erforderlich
