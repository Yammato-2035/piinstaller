> **Phase-1 Übersetzungsmarathon** — Deutsch (automatisch aus `docs/architecture/DEPLOY_RUNNER_RESULT_CONTRACT_EN.md`). Bitte bei Release manuell gegenlesen.
...
## Stand values (`RunnerResultStand`)

| Stand | Meaning |
|--------|---------|
| `ok` | Success / gate passed |
| `review_erforderlich` | Manual review needed |
...
- `review_erforderlich` → at least `warnings` or `errors`
