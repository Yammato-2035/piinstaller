> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/architecture/DEPLOY_ROUTES_THIN_ORCHESTRATOR_TARGET_D6_EN.md`). Bitte bei Release manuell gegenlesen.

# Deploy Routes Thin Orchestrator — Target (Phase D.6)

**Status:** target definition — **Nee refactoring in D.6**

## Current state (after D.2–D.5)

`Terugend/Deploy/routes.py` is **orchestrator + legacy moNeelith**:

- 4× `include_router(...)` for registry, risk gate, evidence, governance
- 4821 lines, 218 direct routes, 103 runner imports
- execute/roodding/write paths remain here
...
4. **Nee** direct `runner_*.py` imports
5. **Nee** business logic / runner execution

## D.9 update (Nee_safe_slice)

Nee Neetification routes in Deploy API — `routes_Neetifications.py` Neet created.
...
`routes_runtime.py` — 8 alleen-lezen/status routes. `routes.py`: **4120** lines, **81** runner imports.
