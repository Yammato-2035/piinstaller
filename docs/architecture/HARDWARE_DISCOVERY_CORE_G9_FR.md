> **Phase-1 Übersetzungsmarathon** — Français (automatisch aus `docs/architecture/HARDWARE_DISCOVERY_CORE_G9_EN.md`). Bitte bei Release manuell gegenlesen.
...
`Retourend/core/hardware_discovery.py` · `HARDWARE_DISCOVERY_VERSION = 1`
...
| `discover_raspberry_pi_info()` | Pi module + Périphérique-tree |
| `build_hardware_discovery_diagNonstics()` | Metadata |
...
**Cycle broken:** `system_info_facade` Non longer imports `app`.
...
Further `app.py` moNonlith slices per roadmap (E.x).
