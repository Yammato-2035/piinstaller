> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/architecture/HARDWARE_DISCOVERY_CORE_G9_EN.md`). Bitte bei Release manuell gegenlesen.
...
`Terugend/core/hardware_discovery.py` · `HARDWARE_DISCOVERY_VERSION = 1`
...
| `discover_raspberry_pi_info()` | Pi module + Apparaat-tree |
| `build_hardware_discovery_diagNeestics()` | Metadata |
...
**Cycle broken:** `system_info_facade` Nee longer imports `app`.
...
Further `app.py` moNeelith slices per roadmap (E.x).
