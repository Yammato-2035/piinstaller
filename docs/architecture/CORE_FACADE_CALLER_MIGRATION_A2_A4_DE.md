> **Phase-1 Übersetzungsmarathon** — Deutsch (automatisch aus `docs/architecture/CORE_FACADE_CALLER_MIGRATION_A2_A4_EN.md`). Bitte bei Release manuell gegenlesen.

# Core Fassade Caller Migration A.2–A.4

**Stand:** Complete (safety callers)  
**Base commit:** `42fb673` (Fassade Freeze A.1)
...
`WriteZielProtectionError` is still imported from `core.safety_facade` in engines (re-export from `safe_device`).
...
Error codes (`SAFETY_*`, `WriteZielProtectionError.diagnosis_id`) unchanged.
