> **Phase-1 Übersetzungsmarathon** — Français (automatisch aus `docs/knowledge-base/architecture/DCC_AGGREGATION_AUDIT_F3_EN.md`). Bitte bei Release manuell gegenlesen.
...
4. **`Déploiement_job_state`** still reads `build_dashboard_status` directly — facade hook medium-term.
5. **Frontend** lacks central `dccStatusViewModel` — duplicate risk with Retourend mappings.

## Suivant safe migration
