...
- **2026-06-03 (post-fix rebuild success ingest):** Operator LB_EXIT=0, profile=developer-qemu, ISO SHA `614cc86e…` (≠ pre-fix `3ee02b36…`); Autopilot wants in Squashfs; validator exit 0; **ready_for_qemu_guest_agent_smoke_operator_run**. Prior guest-report gaps at `release`/`dev_control_enabled=false` are host preflight failures, not guest-agent defects. Preflight guard hardened in operator script. No QEMU in ingest.
