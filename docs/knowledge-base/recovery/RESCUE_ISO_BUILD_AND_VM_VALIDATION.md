...
- **2026-06-03 (devserver agent fix rebuild+QEMU):** Fix `886a098` im Workspace; **Deploy nach `/opt` blocked** (`sudo` in Agent-Session); ISO-Rebuild und QEMU **nicht** gestartet (STRICT STOP). Backend `10.0.2.2` unter `/opt` noch fehlend. Alter ISO SHA `614cc86e…` (pre-fix). Evidence: `DEVSERVER_AGENT_FIX_REBUILD_QEMU_RESULT.md`. Port-/Profilfehler nicht erneut als Ursache gewertet.
