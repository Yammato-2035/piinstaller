...
- **2026-06-04 (QEMU 143148):** ISO-Rebuild+Smoke; Serial `SEND_*` ok; Gast-CLI **GLIBC_2.38** auf Rescue-venv; `guest_report_received=false`. Evidence: `QEMU_143148_FAILURE_CLASSIFICATION.md`. USB gesperrt.
...
- **2026-06-06 (UEFI-x64 Operator-Patch Erfolg):** Kanonische ISO `--in-place` gepatcht; Validator Exit **0**; SHA256 `09b9482a…`; BIOS+EFI El Torito+BOOTX64+efi.img. Classification: `uefi_x64_iso_ready_for_usb_operator_write`. **Kein erneuter UEFI-Patch** solange Validator grün. Next: Operator USB-Write + MSI-Boot. Evidence: `RESCUE_ISO_UEFI_PATCH_OPERATOR_RESULT.md`.
- **2026-06-06 (MSI partial UEFI boot):** Stick bootet auf MSI i7; iwlwifi-9000 + Intel BT (`ibt-17-16-1.sfi`) fehlten im alten Image; `setuphelfer-serial-boot-markers.service` FAILED (QEMU-only Unit). Fix: Paketliste `firmware-iwlwifi`, `firmware-intel-sound`, `wireless-regdb`, `network-manager`; Archive-Areas `main contrib non-free-firmware`; Serial-Marker `ConditionVirtualization=qemu`. Intel-BT (`intel/ibt-*`) über **`firmware-iwlwifi`**, nicht `firmware-intel-sound`. Evidence: `RESCUE_USB_MSI_BOOT_FIRMWARE_AND_SERIAL_MARKER_TRIAGE.md`, `RESCUE_ISO_NON_FREE_FIRMWARE_COMPONENT_FAILURE.md`.
- **2026-06-05 (UEFI-x64 boot gap):** Aktuelle Hybrid-ISO nur ISOLINUX/BIOS (`RESCUE-UEFI-003`); MSI W11 UEFI-Boot fehlgeschlagen. Fix: `patch-rescue-iso-uefi-x64.sh` + `validate-rescue-iso-uefi-boot.sh`. **Behoben** durch Operator-Patch 2026-06-06.
