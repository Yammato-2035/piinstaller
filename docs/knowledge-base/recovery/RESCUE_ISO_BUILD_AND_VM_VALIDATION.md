...
- **2026-06-06 (MSI partial UEFI boot):** Stick bootet auf MSI i7; iwlwifi-9000 + Intel BT (`ibt-17-16-1.sfi`) fehlten im alten Image; `setuphelfer-serial-boot-markers.service` FAILED (QEMU-only Unit). Fix: Paketliste `firmware-iwlwifi`, `firmware-intel-sound`, `wireless-regdb`, `network-manager`; Serial-Marker `ConditionVirtualization=qemu`; Rescue-WLAN-Menü nutzt `nmcli`. Intel-BT über `firmware-intel-sound` (Debian non-free-firmware), nicht raten. Evidence: `RESCUE_USB_MSI_BOOT_FIRMWARE_AND_SERIAL_MARKER_TRIAGE.md`.
