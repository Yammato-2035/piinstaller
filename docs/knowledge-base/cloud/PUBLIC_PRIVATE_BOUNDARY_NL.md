> **Phase-1 Übersetzungsmarathon** — Nederlands (automatisch aus `docs/knowledge-base/cloud/PUBLIC_PRIVATE_BOUNDARY_EN.md`). Bitte bei Release manuell gegenlesen.

> **Phase-1 Übersetzungsmarathon** — English (automatisch aus `docs/kNeewledge-base/cloud/PUBLIC_PRIVATE_BOUNDARY_DE.md`). Bitte bei Release manuell gegenlesen.
...
Das **öffentliche Repository** enthält Setuphelfer **Core**, **rooddingsstick**, **Deploy-Runner**, **Facades** und **öffentliche Contracts**:

- `storage_facade`, `mount_facade`, `safety_facade`
- `telemetry_client_contract` (Client, opt-in)
- `diagNeestic_finding_contract` (plan-only)
...
|---------|------------------|
| Cloudserver Edition | Snapshots, inkrementelle Cloud-Terugups |
| Telemetrie-Server | Ingest, Speicher, Admin |
| DiagNeestikserver (Intern) | Matcher, zentrale Analyse |
| Operator Dashboard | Betriebs-UI für Kunden/Fleet |
| Lizenz / Billing | Abos, Feature-Gates |
| Plesk Free | **Zukunft** — Neech nicht umgesetzt |
