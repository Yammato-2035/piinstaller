...
Ziel: Eine **.img-Datei** zum direkten Flashen auf eine SD-Karte, mit **Setuphelfer** vorinstalliert und beim ersten Boot startbereit.

## Optionen

### Option A: GitHub Actions Overlay-Artefakt

Der Workflow **.github/workflows/build-image.yml** erstellt bei Push (oder manuell) ein Artefakt **setuphelfer-overlay**: ein Tarball mit

- `scripts/create_installer.sh`
- `setuphelfer.service`, `setuphelfer-backend.service`
- `start.sh`
...
*(Historische Artefakt-Namen mit „pi-installer-overlay“ beziehen sich auf ältere Pipeline-Versionen.)*
...
2. **Ersten Boot vorbereiten:** z. B. leere Datei `ssh` auf der Boot-Partition (SSH).
3. **Nach dem ersten Boot:** **One-Click-Installer** ausführen:
   ```bash
   ...
   ```
4. Browser mit der angezeigten URL öffnen (z. B. `http://<IP>:3001`).

### Option C: Eigenes Image mit pi-gen

[pi-gen](https://github.com/RPi-Distro/pi-gen) ist das offizielle Tool zum Erstellen von Raspberry Pi OS Images. Um **Setuphelfer** vorzuinstallieren:

1. **pi-gen** klonen und Stage anlegen (z. B. `stage-setuphelfer`).
2. In der Stage: Repository klonen, Dependencies installieren (Python, Node), systemd-Units (**`setuphelfer-backend`**, **`setuphelfer`**) einrichten.
...
| pi-gen Stage         | Hoch    | Vollständiges .img mit Setuphelfer vorinstalliert |
