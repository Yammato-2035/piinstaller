...
# BR-001: systemd ReadWritePaths für externales Backup-Ziel
...
## Warum externale Backup-Ziele per `ReadWritePaths` freigegeben werden

Backup-Tar und der isolierte Runner schreiben als Nutzer **`setuphelfer`** auf ein **externales** Volume. Ohne Eintrag in **`ReadWritePaths=`** sieht der Dienst Mounts unter `/media/...` nicht zuverlässig — die API meldet dann z. B. **`BACKUP-TARGET-NOT-WRITABLE-002`** (`os.access`), obwohl Unix-Rechte am Mount stimmen.

## Warum nur `/media/setuphelfer/br001`

- **Enger Scope:** Nur der freigegebene BR-001-Zielpfad, nicht ganz `/media` oder `/`.
- Entspricht der Projektregel: externales Ziel **`/media/setuphelfer/br001`** auf **`/dev/sda1`**, nicht Root-FS.
...
Dieser Pfad liegt auf dem **internalen** System-Dateisystem und ist durch Storage-Schutz zu Recht blocked. BR-001 verlangt ein **externales** Medium.
...
- externales Ziel: `docs/knowledge-base/storage/externalal-backup-target-architecture.md`
