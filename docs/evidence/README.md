...
**STRICT Backup/Verify-Kette:** BR-004 und BR-005 gelten nur dann als abgenommen, wenn sie **dasselbe Archiv** prüfen, das BR-001 auf einem **freigegebenen** externen Ziel erzeugt hat (siehe `docs/testing/BACKUP_RESTORE_TEST_MATRIX.md`, `docs/evidence/release-gates/backup_restore_release_gate.json`). **Hinweis 2026-05-12:** Produktions-Backend unter `127.0.0.1:8000` kann denselben Pfad wie die interaktive Shell ablehnen, wenn der Dienstnutzer `/media/<user>` nicht traversieren darf — siehe `docs/evidence/backup-restore/BR-001.json`.
