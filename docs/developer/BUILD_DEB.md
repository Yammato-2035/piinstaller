...
- Bei jedem **Release** (Tag z. B. `v1.3.8.2`) baut der Workflow `.github/workflows/release-deb.yml` automatisch das .deb und hängt es am Release an, inkl. Datei **SHA256SUMS**.
- **Installation mit Verifikation:**
  ```bash
  # Beispiel für v1.3.8.2 (URL an gewünschte Version anpassen)
  RELEASE="v1.3.8.2"
  BASE="https://github.com/Yammato-2035/piinstaller/releases/download"
  wget "$BASE/$RELEASE/pi-installer_1.3.8.2-1_all.deb"
  wget "$BASE/$RELEASE/SHA256SUMS"
  sha256sum -c SHA256SUMS
  sudo apt install ./pi-installer_1.3.8.2-1_all.deb
  ```
...
2. Commit, Push, dann auf GitHub ein **Release** erstellen (Tag z. B. `v1.3.8.2`).
