...
def resolve_debug_log_path(preferred_path: Optional[str] = None) -> str:
    ...
    - Wenn preferred_path gesetzt: nutze ihn; stelle sicher, dass Parent-Dir existiert oder erstellbar ist.
    ...
    if preferred_path and preferred_path.strip():
        ...
        try:
            ...
            if os.access(p.parent, os.W_OK):
                return str(p)
            # Parent nicht schreibbar – trotzdem Pfad zurückgeben (Aufrufer kann fehlschlagen)
            return str(p)
