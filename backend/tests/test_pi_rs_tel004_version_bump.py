"""PI-RS-TEL-004 version bump verification."""
...
class Tel004VersionBumpTests(unittest.TestCase):
    def test_project_version_is_1_9_19_5(self) -> None:
        root = Path(__file__).resolve().parents[2]
        version_json = json.loads((root / "config/version.json").read_text(encoding="utf-8"))
        self.assertEqual(version_json["project_version"], "1.9.19.5")
        self.assertEqual((root / "VERSION").read_text(encoding="utf-8").strip(), "1.9.19.5")
