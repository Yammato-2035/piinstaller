...
DIAGNOSTIC_CATALOG: list[DiagnosticCase] = [
    ...
    DiagnosticCase(id="RESTORE-TMPFS-007", domain="storage_filesystem", title_de="Restore in /tmp fehlgeschlagen", title_en="Restore on /tmp failed", summary_de="/tmp (tmpfs) reicht fuer grosse Restore-Daten oft nicht aus.", summary_en="/tmp (tmpfs) often lacks space for larger restores.", severity="medium", confidence="high", detection_sources=["file_check"], tags=["tmpfs", "storage"]),
