"""NVMe install-target identity and stable role binding (PI-RS-ASUS-WIN11-LINUX-001).
...
def bind_role_by_identity(
    ...
    *,
    ...
    role = role if role in {ROLE_WINDOWS, ROLE_LINUX, ROLE_UNASSIGNED} else ROLE_UNASSIGNED
...
def resolve_role_after_device_rename(
    ...
    win_hash = str((targets.get("windows_target") or {}).get("serial_hash") or "")
    lin_hash = str((targets.get("linux_target") or {}).get("serial_hash") or "")
    win = bind_role_by_identity(inventory, serial_hash=win_hash, role=ROLE_WINDOWS) if win_hash else {
        "ok": False,
        "error": "windows_serial_missing",
        "entry": None,
    }
    lin = bind_role_by_identity(inventory, serial_hash=lin_hash, role=ROLE_LINUX) if lin_hash else {
        "ok": False,
        "error": "linux_serial_missing",
        "entry": None,
    }
