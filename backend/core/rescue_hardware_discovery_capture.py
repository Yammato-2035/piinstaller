...
def collect_nvme_health(*, inventory: Mapping[str, Any], run: RunFn | None = None) -> dict[str, Any]:
    ...
    for entry in inventory.get("devices_redacted") or []:
        ...
        if ctrl:
            ...
            smart_raw.append({"path": ctrl, "rc": rc, "smart": smart, "stderr": err[:500] if err else ""})
            rc, out, err = run(["nvme", "error-log", ctrl, "--log-entries=64", "-o", "json"], 60.0)
            err_log = _parse_json(out) if rc == 0 else {"rc": rc, "error": err or out}
            error_raw.append({"path": ctrl, "error_log": err_log})
        if ns:
        ...
        devices_out.append(
            {
                "nvme_identity_hash": entry.get("nvme_identity_hash"),
                "device_path": ns,
                "controller": ctrl,
                "firmware_revision": entry.get("firmware_revision"),
                "smart": {
                    "critical_warning": (smart or {}).get("critical_warning") if isinstance(smart, dict) else None,
                    "composite_temperature": (smart or {}).get("temperature") if isinstance(smart, dict) else None,
                    "available_spare": (smart or {}).get("avail_spare") if isinstance(smart, dict) else None,
                    "available_spare_threshold": (smart or {}).get("spare_thresh") if isinstance(smart, dict) else None,
                    "percentage_used": (smart or {}).get("percent_used") if isinstance(smart, dict) else None,
                    "data_units_read": (smart or {}).get("data_units_read") if isinstance(smart, dict) else None,
                    "data_units_written": (smart or {}).get("data_units_written") if isinstance(smart, dict) else None,
                    "host_read_commands": (smart or {}).get("host_read_commands") if isinstance(smart, dict) else None,
                    "host_write_commands": (smart or {}).get("host_write_commands") if isinstance(smart, dict) else None,
                    "controller_busy_time": (smart or {}).get("controller_busy_time") if isinstance(smart, dict) else None,
                    "power_cycles": (smart or {}).get("power_cycles") if isinstance(smart, dict) else None,
                    "power_on_hours": (smart or {}).get("power_on_hours") if isinstance(smart, dict) else None,
                    "unsafe_shutdowns": (smart or {}).get("unsafe_shutdowns") if isinstance(smart, dict) else None,
                    "media_errors": (smart or {}).get("media_errors") if isinstance(smart, dict) else None,
                    "num_err_log_entries": (smart or {}).get("num_err_log_entries") if isinstance(smart, dict) else None,
                    "warning_temp_time": (smart or {}).get("warning_temp_time") if isinstance(smart, dict) else None,
                    "critical_comp_time": (smart or {}).get("critical_comp_time") if isinstance(smart, dict) else None,
                    "temperature_sensors": {
                        "sensor_1": (smart or {}).get("temperature_sensor_1") if isinstance(smart, dict) else None,
                        "sensor_2": (smart or {}).get("temperature_sensor_2") if isinstance(smart, dict) else None,
                    },
                    "firmware_revision": entry.get("firmware_revision"),
                },
...
def run_hardware_discovery(
    *,
    ...
    _write_json(out_dir / "machine_identity.json", {k: v for k, v in identity.items() if "serial" not in k.lower() or "masked" in k.lower() or "hash" in k.lower()})
    _write_json(
        out_dir / "machine_binding.json",
        {
            "ok": binding_ok,
    ...
    if not binding_ok and not allow_non_gabriel_diag:
        ...
        _write_json(out_dir / "PHYSICAL_DIAGNOSIS_RESULT.json", result)
        return result

    captures = dict(status["captures"])
    captures["machine_identity"] = "complete"
    captures["bios_inventory"] = "complete"
    bios = {
    ...
    # Strip serials from published raw pointer
    _write_json(
    ...
    captures["nvme_inventory"] = "complete" if inventory.get("device_count") else "partial"
    ...
    captures["nvme_health"] = "complete" if health.get("devices") else "partial"
    ...
    captures["windows_setup_logs"] = "complete"

    cause = build_cause_matrix(nvme_health=health, kernel=kernel, windows_logs=win_logs, bios_status=bios["status"])
    _write_json(out_dir / "cause_matrix.json", cause)

    capture_complete = all(v == "complete" for v in captures.values()) and bool(inventory.get("device_count") >= 2)
    # Identity fields: require serial hash on both for "complete" physical identity
    serials_ok = all(d.get("serial_hash") for d in (inventory.get("devices_redacted") or []))
    if not serials_ok:
    ...
    # role binding stub (no write auth)
    ...
    for path in sorted(out_dir.rglob("*")):
        if path.is_file() and path.name != "sha256sums.txt":
            ...
            # Skip protected raw serials from published sums? Include but path marks protected
    ...
    (out_dir / "PHYSICAL_DIAGNOSIS_RESULT.md").write_text("\n".join(md_lines), encoding="utf-8")
