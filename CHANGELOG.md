...
- **Rescue ISO / Legacy-rsvg:** `rsvg-convert` aus `librsvg2-bin` wird nicht mehr fälschlich als komplett fehlende Host-Abhängigkeit behandelt. Wenn `live-build` den Legacy-Befehl `rsvg` erwartet, unterscheidet der Preflight jetzt sauber zwischen `blocked_build_tools_missing` und `blocked_legacy_rsvg_command_missing`; projektlokaler Wrapper-Support unter `build/rescue/tool-compat/bin/rsvg`, kein globaler `/usr/bin/rsvg`-Symlink.
- **Rescue ISO / Zielarchitekturen:** Dashboard-State und Evidence trennen jetzt Host-Architektur, Kandidaten (`amd64`, `i386`) und separate Deferred-Tracks (`arm64`, `armhf`). Kein Ziel wird ohne Build-/Boot-Evidence als final grün markiert; USB-Write bleibt blockiert.
- **Rescue ISO / erster echter amd64-Buildversuch:** Kontrollierter `lb build` für `amd64` wurde ausgeführt, aber vom vorhandenen `auto/build`-Gate mit Exit `20` bewusst gestoppt; jetzt präzise als `blocked_controlled_build_gate_required` statt als generischer Live-Build-Konfigurationsfehler klassifiziert, ohne USB-Write, ohne `sudo`-Fallback und ohne zweiten Build-Versuch.
- **Rescue ISO / gate-konformer Wrapper-Lauf:** Kontrollierter `amd64`-Build über `scripts/rescue-live/run-controlled-iso-build-with-logging.sh --operator-confirm-build` wurde versucht, aber vor dem eigentlichen ISO-Build durch `sudo` ohne Terminal/Passwort gestoppt; sauber als `blocked_requires_operator_sudo_policy` klassifiziert, ohne unsafe Folgeaktion.
...
- Doku/KB/Evidence/i18n: u. a. `DEPLOY_RESCUE_SANDBOX_CONTROLLED_COPY_{DE,EN}.md`, `DEPLOY_RESCUE_BUILD_ENVIRONMENT_EMULATION_{DE,EN}.md`, KB-Artikel und FAQ-Ergï¿½nzungen.
...
- `POST /api/backup/create` (`type=data`) gibt Source-Planung nun auch im Erfolgspfad zurï¿½ck (`selected_sources`, `skipped_sources`, `required_sources`, `optional_sources`).
- `POST /api/backup/restore` (`mode=preview`) ergï¿½nzt PrivateTmp-Kontext (`private_tmp_isolation`, `preview_dir_visibility_note`, `service_private_tmp_hint`).
- i18n ergï¿½nzt: `backup.messages.preview_private_tmp_hint` (DE/EN).

### Fixed (Backup API - FIX-10 backup/list stability hardening)
- `/api/backup/list` auf read-only Pfad validiert (ohne Write-Probe), weiterhin harte Blockade fï¿½r `/media` / `/run/media`.
...
- Strukturierter Timeout-Code fï¿½r Listen-Endpunkt: `backup.list_timeout` inkl. `details.command`, `details.timeout_seconds`, `details.target`.
- Dateidatum ohne externe `date`-Subprozesse (reines `datetime.fromtimestamp`), um zusï¿½tzliche Blockadequellen zu entfernen.
- i18n ergï¿½nzt: `backup.messages.list_timeout` (DE/EN).

### Fixed (Backup API - FIX-11 backup/list decouple via index)
- `GET /api/backup/list` ist vom direkten Directory-Scan entkoppelt und liest primï¿½r aus einem lokalen Backup-Index (`backup-index.json` im State-Verzeichnis).
- Erfolgreiche Backup-Lï¿½ufe aktualisieren den Index mit Metadaten (`backup_file`, `created_at`, `encrypted`, `size_bytes`, `type`, `target`, `source_summary`, `manifest_present`, `verification_status`, `storage_path`) ohne Secrets.
...
- Optionale Existenzprï¿½fung pro Indexeintrag ist kurz getaktet (`quick_stat`); bei Timeout bleibt der Eintrag sichtbar mit `status=unknown`.
...
- `POST /api/backup/restore` trennt `mode=preview` und `mode=restore` jetzt strikt; `mode=restore` lï¿½uft nicht mehr durch den Preview-Pfad.
- API-Vertrag geschï¿½rft: bei `mode=restore` ist `target_dir` Pflicht (`backup.restore_target_missing`), `target_dir="/"` wird hart blockiert.
- Restore-Ziele werden bei `mode=restore` deterministisch validiert; ungï¿½ltige Ziele liefern `backup.restore_target_invalid`, nicht beschreibbare Ziele `backup.restore_not_writable`.
- Erfolgs-Code fï¿½r echten Restore ergï¿½nzt: `backup.restore_success`; Preview bleibt `backup.restore_preview_ok`.
...
### Fixed (Backup API ï¿½ FIX-2 sudo gate, HW1 / NoNewPrivileges)
...
- Tests ergï¿½nzt: `backend/tests/test_backup_data_source_scope_v1.py`.

### Fixed (Backup Runtime - FIX-4 source trace + path unification, HW1)
- Runtime-Mismatch behoben: aktiver `type=data`-Pfad nutzt nicht mehr die Altliste `/home /var/www /opt`, sondern ausschlieï¿½lich Source-Planning.
- Harte Ausschlussregel fï¿½r Data-Backups ergï¿½nzt: gesamter `/opt`-Tree (inkl. `/opt/containerd`) wird in `type=data` nie als `selected_sources` akzeptiert.
- Strukturierte Trace-Logs vor Tar-Lauf ergï¿½nzt (`backup_type`, `target`, `selected_sources`, `skipped_sources`, `required_sources`, `optional_sources`, `effective_tar_command`).
...
- `type=data` verwendet als Pflichtquelle ausschlieï¿½lich das Home des **effektiven Dienstnutzers** (`pwd.getpwuid(os.geteuid()).pw_dir`) statt implizitem Login-Shell-Kontext.
...
- Fï¿½r reproduzierbare HW-Lï¿½ufe unterstï¿½tzt `type=data` jetzt explizite Pflichtquellen ï¿½ber `SETUPHELFER_DATA_BACKUP_SOURCES` (z. B. `/mnt/setuphelfer/test-data`).
- Ist die Variable gesetzt, ersetzt sie die Default-Pflichtquelle (Service-Home) vollstï¿½ndig; `/home` ist dann keine implizite Voraussetzung fï¿½r HW-Success.
...
### Fixed (Backup / Recovery ï¿½ file-based engine hardening)
- `backend/modules/backup_engine.py`: file-based Backup archiviert jetzt Dateien **und** Verzeichnisse rekursiv mit relativen, kollisionsgeschï¿½tzten Archivpfaden; ï¿½berlappende Eingaben werden dedupliziert und im Manifest dokumentiert (`skipped_inputs`).
- `backend/modules/backup_verify.py`: Verify blockiert unsichere Archiv-Member (Traversal, absolute Pfade, Link-/Sondertypen) und prï¿½ft Manifestpfade konsistent zur extrahierten Struktur.
...
- `backend/tests/test_backup_recovery_engines.py`: Tests fï¿½r rekursive Verzeichnisarchivierung, relative Pfade, Kollisionsfehler, Restore-Pfadstruktur, Manifest/Archiv-Inkonsistenz und Allowlist-Schutz erweitert.
...
### Notes
- Die File-Engine-Hï¿½rtung verbessert die technische Wiederherstellbarkeit, ersetzt aber keinen echten Full-Recovery-Nachweis mit VM-Reboot und anschlieï¿½endem Hardware-Test.
...
- Deterministischer Restore-Pfad ohne manuelle API-Workarounds; verpflichtendes **Manifest** im Archiv (Fail-Fast bei fehlendem oder ungï¿½ltigem `MANIFEST.json`).
- Zweisprachige Backend-Texte fï¿½r `backup_recovery_i18n` ï¿½ber Locale (`SETUPHELFER_LANG` / `LC_MESSAGES` / `LANG` mit Prï¿½fix `de`) sowie ergï¿½nzte Frontend-Keys (`backup.messages.*`, `verify.messages.*`, `restore.messages.*`).
...
- **systemd**-Units und Installer-Flow stabilisiert (Speicherlimits, Capabilities, Finalprï¿½fung aktiver Dienste nach `install-system.sh`).
- Dokumentation zu Backup-/Recovery-Engines und Manifest um deterministisches Verhalten und englische Kurzabschnitte ergï¿½nzt.

### Fixed

- Restore-/Boot-Themenkreis in Doku und FAQ konsolidiert; fehlende i18n-Zuordnung fï¿½r u. a. `backup.failed_manifest_missing` / Archiv-Integritï¿½t in der Web-UI.

---

## [1.4.1.0] - 2026-04-07

### Added (Backup / Recovery ï¿½ Engines & Doku)
...
- **CLI:** `recovery/main.py` (Recovery-Menï¿½ bei unvollstï¿½ndigem Root).
- **Tests:** `backend/tests/test_backup_recovery_engines.py` (Simulation Zerstï¿½rung/Wiederherstellung in `/tmp`).
...
- **Hinweis:** Schlï¿½ssel fï¿½r Verschlï¿½sselung werden **nicht** ins Backup geschrieben; Boot-Tests auf echter Hardware weiterhin manuell.

---

## [1.4.0.5] - 2026-04-07

### Changed (Betrieb ï¿½ Setuphelfer-Standard, PI_INSTALLER Legacy)
- **`scripts/deploy-to-opt.sh`:** `mkdir -p` fï¿½r **`STATE_DIR`** vor `chown`; Texte/Titel Setuphelfer; Hinweis zu Unit-Platzhaltern `{{PI_INSTALLER_*}}`.
- **`scripts/install-system.sh`:** Banner/Fehlertexte Setuphelfer; **`SETUPHELFER_USE_SERVICE_USER`** / **`SETUPHELFER_USER`** mit Fallback auf `PI_INSTALLER_*`; **`chown`/`chmod`** fï¿½r **`STATE_DIR`**; Symlink **`setuphelfer` ? `start-setuphelfer.sh`**; **`profile.d`:** Primï¿½r `SETUPHELFER_*`, `PI_INSTALLER_*` nur als Spiegel aus denselben Werten.
- **`backend/core/install_paths.py`:** `_env_path_setup_then_legacy` ï¿½ explizit SETUPHELFER vor PI_INSTALLER (Bugfix: `get_state_dir` nutzte fï¿½lschlich `_env_path_first`).
- **`scripts/install-backend-service.sh`**, **`create_installer.sh`:** `SETUPHELFER_*`-Variablen zuerst; **`sudo mkdir -p`** fï¿½r Config/Log/State vor Service-Install.
- **`scripts/start-setuphelfer.sh`:** **`SETUPHELFER_MODE`** vor **`PI_INSTALLER_MODE`**.
- **`scripts/uninstall-system.sh`:** Kopfkommentar Setuphelfer.
- **`docs/architecture/NAMING_AND_SERVICES.md`:** ENV-Reihenfolge prï¿½zisiert.

---

## [1.4.0.4] - 2026-04-07

### Added (Phase F ï¿½ Zielsystem-Prï¿½fung)
- **`scripts/verify-setuphelfer-install.sh`** ï¿½ Prï¿½ft **systemd** (`setuphelfer-backend` / `setuphelfer`, Legacy `pi-installer*`), **curl** auf `/api/version` und Web-UI :3001, optional **journalctl**; Exit-Code 0/1.
- **`docs/VERIFY_TARGET_SYSTEM.md`** ï¿½ Kurzanleitung und manuelle Kommandos; Verweise in `NAMING_AND_SERVICES.md`, `architecture/README.md`, `SYSTEM_INSTALLATION.md`.

---

## [1.4.0.3] - 2026-04-07

### Changed (Phase D4 ï¿½ Changelog-, Versions- und FAQ-Pflege)
- **Workflow:** `.cursor/rules/projekt-workflow.mdc` ï¿½ einheitliche Quelle **`config/version.json`**, Sync per `sync-version.js`; Changelog-Pflicht; systemd **setuphelfer-backend** in Abschnitt 6.
- **Doku:** `docs/developer/VERSIONING.md` ï¿½ Titel Setuphelfer, Abschnitt **ï¿½Release-Pflicht fï¿½r Maintainer (Phase D4)ï¿½** (Checkliste: version.json, sync, CHANGELOG.md, In-App-Changelog, FAQ, debian/changelog).
- **In-App:** FAQ-Eintrag **ï¿½Version und Changelog ï¿½ wo wird was gepflegt?ï¿½** fï¿½r Mitwirkende; Versionsblock **1.4.0.3**.

---

## [1.4.0.2] - 2026-04-07

### Changed (Architektur & Review ï¿½ Phase D3)
- **Referenz:** `docs/architecture/NAMING_AND_SERVICES.md` ï¿½ Produktname, Debian-Source vs. Binary-Paket `setuphelfer`, Pfade, systemd-Units, ENV (`SETUPHELFER_*` / `PI_INSTALLER_*`), Tauri-Binary-Hinweis.
- **Index:** `docs/architecture/README.md` ï¿½ Einstieg in Architektur-Dokumente.
...
### Changed (Dokumentation ï¿½ Phase D2)
- **Nutzerdoku:** `docs/START_APPS.md`, `docs/BETRIEB_REPO_VS_SERVICE.md`, `docs/SYSTEM_INSTALLATION.md`, `docs/SD_CARD_IMAGE.md` auf **Setuphelfer**-Pfade und **systemd**-Units (`setuphelfer.service`, `setuphelfer-backend.service`) ausgerichtet; Legacy-Hinweise fï¿½r `pi-installer` &lt; 1.4.0 wo nï¿½tig.
- **In-App:** `Documentation.tsx` ï¿½ FAQ (Starter, Pfade), Versionsblock **1.4.0.1**; **README** ï¿½ Kurzverweis auf `/opt/setuphelfer`.
- **Skripte:** `install-system.sh` ï¿½ Zusammenfassung nach Installation (Befehle, systemd, Startmenï¿½); `uninstall-system.sh` ï¿½ Entfernen der Symlinks **`setuphelfer*`**, Desktop-Dateien **`setuphelfer*.desktop`**, optionaler User **`setuphelfer`**.

---

## [1.4.0.0] - 2026-04-08

### Changed (Breaking ï¿½ Betrieb)
...
### Fixed
- **Service-Topologie:** Port **8000** gehï¿½rt nur noch **`pi-installer-backend.service`**. `scripts/start-browser-production.sh` startet kein Uvicorn mehr; bei fehlendem API klare Fehlermeldung und Exit.
...
- **APP_EDITION:** Explizit **`release`** in Unit-Vorlagen (zusï¿½tzlich zu DEB); Pfade **`PI_INSTALLER_*`** per Platzhalter in beiden Vorlagen.
...
- **Deploy:** `scripts/deploy-to-opt.sh` fï¿½hrt `npm run build` aus und schreibt die gleiche `ExecStart`-Zeile.
...
### Changed
- **Backup & Restore:** Ampel auf **Freigabelage Phase 3** ausgerichtet (kein Grï¿½n ohne nachgewiesenen Backup-Lauf; Gelb bei ï¿½Kern nicht verifiziertï¿½; Rot bei `sudo_required` bzw. Diagnose/Restore-Vorschau-Risiko); Pflicht-Hinweistext per i18n.
- **Dokumentation:** `docs/developer/BACKUP_RESTORE_PHASE3_RELEASE_ASSESSMENT.md` ergï¿½nzt/verwendet; Verifikationsnotiz Phase 2D/2E referenziert.
...
### Added
- **Diagnose (Architektur):** Kanonisches ï¿½bergangsschema `localization_model` (`legacy` \| `key_v1`), i18n-Key-Felder (`title_key`, `user_message_key`, `suggested_action_keys`), `diagnosis_code`, Referenzen (`docs_refs`, ?) in `backend/models/diagnosis.py`; Doku `docs/architecture/diagnosis_localization.md`.
...
### Changed
- **Frontend:** `DiagnosisPanel` wï¿½hlt Texte per Keys oder Legacy; Typen in `types/diagnosis.ts`; `localBackendDiagnosis` / `localDiagnosisFallback` auf **key_v1**; neue Keys unter `diagnosis.codes.*` in `de.json` / `en.json`.
