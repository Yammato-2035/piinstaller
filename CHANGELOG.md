...
- **PI-RS-ASUS-EMERGENCY-LINUX-TELEMETRY-003 / PI-RS-ASUS-CARRIER-BUILD-WRITE-004 (1.10.2.0):** ASUS-Emergency-Linux-Kampagne — Boot-/Hardware-/Kernel-/Device-Lifecycle-Sentinels, Driver-Failure-Resolver, Boot-Comparison-Engine, ASUS-00…05 + ASUS-RECOVERY Bootprofile, Telemetrie-Spooler, Diagnostik-Forwarding- und Dashboard-Contracts; Live-Build-Integration (systemd-Units unter `/opt/setuphelfer-rescue`, GRUB-Menüeinträge); Payload **1.10.0.17**. Controlled ISO + physischer Carrier-Write nur nach Gates und zwei Operatorbestätigungen. Evidence: `docs/evidence/rescue/asus-emergency-linux-003/`.
- **PI-RS-HW-BASELINE-DIAG-I18N-002 (1.10.1.0):** Frühe, sichere Hardware-Baseline-Diagnostik (RAM/CPU/GPU/HDD/SATA-SSD/NVMe) mit additivem Startup-Gate, read-only/preview-only APIs und Rescue-UI; vollständige Hardware-Dokumentation/FAQ/Wissensdatenbank in DE/EN/FR/NL inkl. i18n-Completeness-Gate (`native_review_pending`). Statusvokabular ohne Garantie-Claims; physische Extended-Tests bleiben `pending_physical_validation`. Endstatus: `implemented_baseline_diagnostics_and_four_language_docs_pending_physical_extended_tests`. Evidence: `docs/evidence/rescue/hardware-baseline-002/`.
- **PI-RS-HW-COMPAT-PROVISION-001:** Neue, parallele Hardware-Erkennungs-, Treiber-/Firmware-Resolver- und Multi-Arch-Provisionierungsschicht für den Rettungsstick — CPU/SoC-, Mainboard/Chipsatz-, GPU-, USB-, Input-, Drucker-/Scanner-Erkennung; generischer Treiber-/Firmware-Resolver mit sicherem Aktivierungsplan (`preview_only`); kuratierter Hardware-Kompatibilitätskatalog (`data/hardware/`); Raspberry Pi 3–5 Modellerkennung/Bootplan/OS-Kompatibilitätsmatrix; 64-GB-Carrier-Kapazitätsplan + Strategieentscheidung (Variante C — Orchestrator-Cache, evidenzbasiert); signierter OS-Imagekatalog + Provisionierungsplan (`write_allowed` immer `false`); 5 neue read-only/preview-only API-Router; neue Rescue-UI-Hardwareübersicht mit Ampelstatus; Telemetrie-Redaction-Contract (`hardware_inventory_summary_v1`) + additive DCC-Statusfelder; 19 neue Testdateien (198+ Tests grün); physische Testmatrix vorbereitet (alle Einträge `planned`, keine physische Verifikation in dieser Phase); Architektur-/Wissensdatenbank-/FAQ-Dokumentation (DE/EN). Keine Duplikation bestehender Hardware-/Storage-/Safety-Logik — siehe `docs/evidence/rescue/hardware-compat-001/HARDWARE_DISCOVERY_IST_AUDIT.md`. Kein Schreibvorgang, keine Treiber-/Firmwareinstallation, kein Image-Download in dieser Phase. Endstatus: `implemented_hardware_inventory_and_provisioning_preview_pending_physical_matrix`.
