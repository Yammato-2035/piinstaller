...
- **PI-RS-ASUS-ROOTCAUSE-006B (1.10.6.0):** TUI-Repro Boot1/Boot2 (`20260807_221550`/`20260808_064943`) = `reproducible`; MCE-Decoder/MODE2/intentional-NVIDIA-Heuristiken korrigiert; severity≠action_impact; ASUS-Fixtures; Restore nicht mehr durch False Positives blockiert. Boot3 + Carrier-Update ausstehend. Evidence: `docs/evidence/rescue/asus-rootcause-006/`.
- **PI-RS-ASUS-ROOTCAUSE-TELEMETRY-006 (1.10.5.0):** Autocapture nutzt Backend-venv (psutil-Fix); read-only Partitionshelfer in TUI + Autocapture (`rescue_partitions_tui_preview`, `write_allowed=false`); CPU/DDR/NVMe-Baseline + Partition-Inventur. Evidence: `docs/evidence/rescue/asus-rootcause-006/`.
- **PI-RS-ASUS-ROOTCAUSE-TELEMETRY-006 (1.10.4.0):** Unattended TUI-Baseline-Autocapture — einmalig Boot-/Runtime-Diagnostics + read-only Hardware-Baseline (`quick`: CPU/DDR-Probe/NVMe SMART-Log, keine NVMe-Writes); GUI-Menü auf Baseline ausgeblendet/gesperrt; `setuphelfer_auto_hw_baseline=1`. Evidence: `docs/evidence/rescue/asus-rootcause-006/`.
- **PI-RS-ASUS-ROOTCAUSE-TELEMETRY-006 (1.10.3.0):** TUI-first Stabilisierung auf ASUS ROG Strix G513QM � Profile `ASUS-TUI-BASELINE` / `ASUS-XORG-FORENSIC` / `ASUS-GUI-CONTROLLED`; kein GUI-Autostart auf TUI-Baseline; Console-Ownership (`fallback_tui`/`unknown`); Port-Ownership; startx-Forensic isoliert; Diagnostics-Timer gated; GRUB `default=0` ? TUI-Baseline. Carrier SquashFS+GRUB Update verifiziert (`4629ca61�` / `15497518�`); physische TUI-Baseline-Boots ausstehend. Evidence: `docs/evidence/rescue/asus-rootcause-006/`.
- **PI-RS-ASUS-EMERGENCY-LINUX-TELEMETRY-003 / PI-RS-ASUS-CARRIER-BUILD-WRITE-004 (1.10.2.0):** ASUS-Emergency-Linux-Kampagne � Boot-/Hardware-/Kernel-/Device-Lifecycle-Sentinels, Driver-Failure-Resolver, Boot-Comparison-Engine, ASUS-00�05 + ASUS-RECOVERY Bootprofile, Telemetrie-Spooler, Diagnostik-Forwarding- und Dashboard-Contracts; Live-Build-Integration (systemd-Units unter `/opt/setuphelfer-rescue`, GRUB-Men�eintr�ge); Payload **1.10.0.17**. Controlled ISO + physischer Carrier-Write nur nach Gates und zwei Operatorbest�tigungen. Evidence: `docs/evidence/rescue/asus-emergency-linux-003/`.
- **PI-RS-HW-BASELINE-DIAG-I18N-002 (1.10.1.0):** Fr�he, sichere Hardware-Baseline-Diagnostik (RAM/CPU/GPU/HDD/SATA-SSD/NVMe) mit additivem Startup-Gate, read-only/preview-only APIs und Rescue-UI; vollst�ndige Hardware-Dokumentation/FAQ/Wissensdatenbank in DE/EN/FR/NL inkl. i18n-Completeness-Gate (`native_review_pending`). Statusvokabular ohne Garantie-Claims; physische Extended-Tests bleiben `pending_physical_validation`. Endstatus: `implemented_baseline_diagnostics_and_four_language_docs_pending_physical_extended_tests`. Evidence: `docs/evidence/rescue/hardware-baseline-002/`.
- **PI-RS-HW-COMPAT-PROVISION-001:** Neue, parallele Hardware-Erkennungs-, Treiber-/Firmware-Resolver- und Multi-Arch-Provisionierungsschicht f�r den Rettungsstick � CPU/SoC-, Mainboard/Chipsatz-, GPU-, USB-, Input-, Drucker-/Scanner-Erkennung; generischer Treiber-/Firmware-Resolver mit sicherem Aktivierungsplan (`preview_only`); kuratierter Hardware-Kompatibilit�tskatalog (`data/hardware/`); Raspberry Pi 3�5 Modellerkennung/Bootplan/OS-Kompatibilit�tsmatrix; 64-GB-Carrier-Kapazit�tsplan + Strategieentscheidung (Variante C � Orchestrator-Cache, evidenzbasiert); signierter OS-Imagekatalog + Provisionierungsplan (`write_allowed` immer `false`); 5 neue read-only/preview-only API-Router; neue Rescue-UI-Hardware�bersicht mit Ampelstatus; Telemetrie-Redaction-Contract (`hardware_inventory_summary_v1`) + additive DCC-Statusfelder; 19 neue Testdateien (198+ Tests gr�n); physische Testmatrix vorbereitet (alle Eintr�ge `planned`, keine physische Verifikation in dieser Phase); Architektur-/Wissensdatenbank-/FAQ-Dokumentation (DE/EN). Keine Duplikation bestehender Hardware-/Storage-/Safety-Logik � siehe `docs/evidence/rescue/hardware-compat-001/HARDWARE_DISCOVERY_IST_AUDIT.md`. Kein Schreibvorgang, keine Treiber-/Firmwareinstallation, kein Image-Download in dieser Phase. Endstatus: `implemented_hardware_inventory_and_provisioning_preview_pending_physical_matrix`.
- **PI-RS-MSI-GUI-002:** Payload **1.10.0.15** � GUI unter MSI-Compat/nomodeset gesperrt; kein openvt/chvt/startx; TUI prim�r; Operator-Meldung; Repack + Content-Check; kein USB-Write.
...
- `docs/roadmap/STATUS_MATRIX.md` � PI-RS-USB-MSI-FIX-001 Stick 1.10.0.14 geschrieben, GE63 boot pending
...
- `docs/roadmap/STATUS_MATRIX.md` � PI-RS-MSI-RETEST-002 Payload 1.10.0.13 stick ready, operator boot pending
...
- `STATUS_MATRIX.md`, `NEXT_STEPS_AFTER_1_9_0_0.md` � F.1 gr�n, F.2 blocked bis Execute-Prompt
...
- _(In 1.8.12.0 zusammengef�hrt � siehe E.13 dort.)_
...
- **Legal/Beta:** Public-safe Checklisten (Beta, NDA, Telemetrie-Einwilligung, TOM, L�schkonzept, AV).
...
- **Fleet finish Exit-1:** Triage � `fleet_session_patch` `${3:-{}}` korrumpiert JSON; Fix `${3-}`; API Finish unver�ndert OK.
...
- **Windows Rescue Telemetry + BitLocker:** Telemetrie-Schema, Server-Vertrag, Store-and-forward-Konzept; BitLocker-Codes 001�006; DCC `WindowsRescueInspectCard`; kein Gr�n ohne Server-ACK. Next Prompt: `WINDOWS11_RESCUE_INSPECT_MVP_OPERATOR_READONLY_SCAN`.
