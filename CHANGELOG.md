...
- Rettungsstick-Paketliste (`setuphelfer.list.chroot`, Quelle: Heredoc in `scripts/rescue-live/prepare-controlled-live-build-tree.sh`) mit `RESCUE_REQUIRED_PACKAGE_POLICY_V2.json` synchronisiert und um `intel-microcode`/`amd64-microcode` ergänzt — schließt eine bestehende Drift (Pakete aus der Policy fehlten im gebauten `build/`-Baum) und behebt nebenbei, dass `validate-live-build-dpkg-preflight.sh` wegen `parted`/`ntfs-3g` (verbotene Pakete, kein Schreib-Gate) aktuell mit `forbidden_package` fehlgeschlagen wäre; beide Pakete bewusst draußen gelassen.
- `rescue_peripheral_discovery.py` (neu): read-only USB-/Eingabe-/Audio-Geräteerkennung für den Rettungsstick (`lsusb`, `/proc/bus/input/devices`, `aplay -l`), als neue `peripherals`-Sektion in `system-assessment.v2`.
- `driver_catalog.py` (neu): kleine, erweiterbare Vendor→Hersteller-Support-URL-Tabelle (GPU/Netzwerk/Eingabegeräte/Mainboard/Drucker), gespiegelt aus der bisherigen Frontend-Liste in `PeripheryScan.tsx` plus Drucker-Hersteller (Canon/Epson/Brother); liefert `driver_hints` im Assessment-Output (plan-only, nur offizielle Hersteller-Domains). Siehe `docs/knowledge-base/rescue/DRIVER_CATALOG_AND_PERIPHERAL_DISCOVERY.md`.
- `docs/architecture/RESCUE_ARM64_RASPBERRY_PI_BOOT_FEASIBILITY.md` (neu, nur Doku): Machbarkeitsskizze für Raspberry-Pi-Rescue-Boot (arm64) — kein Code, kein Build; Pi-Rescue-Boot ist laut Capability-Matrix aktuell `status: missing`.
